...
/**
 * Final Exam Scheduler using Simulated Annealing Algorithm (Cluster version)
 * Usage: java -Dpj.np=<K> FESSAClu <sections-file> <students-file> <relationship-file> [schedule-output-file]
...
public class FESSAClu {
	...
	public static void main(final String[] args) throws Exception {
		...
		// intitiate the algorithm
		final SimulatedAnnealing sa = new SimulatedAnnealing();

		// get distributed of iteration
		final int interations = (int)Math.ceil((double)sa.getPerturbInteration()/(double)world.size());

		// schedule transmit buffer
		final ObjectItemBuf<Schedule> buf = ObjectItemBuf.buffer();

		// loop until freezing temperature is reached
		while(!sa.isDone()) {
			// run iterations
			sa.runIteration(interations);
	...
	public static void usage() {
		System.err.println("Usage: java -Dpj.np=<K> FESSAClu <sections-file> <students-file> <relationship-file> [schedule-output-file]");
